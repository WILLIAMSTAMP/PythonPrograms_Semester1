...
while True:
    ...
    while True:
        try:
            Exp_Date = input("Enter the expiry date:  (MM-YY): ")
            Exp_Date = datetime.datetime.strptime(Exp_Date, "%m-%y")
        except:
            print("Expiry date format is not valid - please re-enter.")
        else:
            break
    ...
    if Cont == "N":
        print("Thank you for using the Honest Harry Sales Program. Goodbye.")
        exit()